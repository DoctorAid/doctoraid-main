...
export const createSlots = async (req, res) => {
    try {
        const { doctorid, startTime, endTime, date, duration, event, pin } = req.body;
        ...
        const new_session = new Session({doctorId:new mongoose.Types.ObjectId(doctor._id),date:date,time:session_time, event:event, pin:pin});   // creating a new session
